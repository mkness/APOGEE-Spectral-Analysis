...
def weighted_quantile(values, weights, quantile):
    ...
    ## bugs:
    - slow
    - quantile in percentage units
    """
    sindx = np.argsort(values)
    cvalues = 1. * np.cumsum(weights[sindx])
    cvalues = cvalues / cvalues[-1]
    indx = (sindx[cvalues > (quantile / 100.)])[0]
    return values[indx]

def get_continuum(dataall, delta_lambda=50., quantile=80.):
    """
    ## inputs:
    ...
    * for loops!
    ...
    for star in range(Nstar):
        ...
        for ll, lam in enumerate(dataall[:, 0, 0]):
            ...
            continuum[ll, star] = weighted_quantile(dataall[indx, star, 1], ivar, quantile)
    return continuum
